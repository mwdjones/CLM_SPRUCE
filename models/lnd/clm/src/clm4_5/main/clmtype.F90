...
type, public :: pft_cflux_type
...
#if (defined MICROBE)
   real(r8), pointer :: m_frootc_to_doc(:)           ! fine root C transfer to dissolved organic carbon (gC/m2/s)
#endif
   ...
   real(r8), pointer :: excess_cflux_to_cpool(:)             ! downregulation flux goes to short-term storage
   real(r8), pointer :: cpool_to_turnover(:)                 ! turnover of short-term storage     
...
type, public :: pft_nflux_type
...
#if(defined MICROBE)
   real(r8), pointer :: m_frootn_to_don(:)               ! fine root N to don (gN/m2/s)
#endif
   ...
   real(r8), pointer :: hrv_npool_to_litter(:)                ! storage N pool harvest mortality (gN/m2/s)
  ...
  real(r8), pointer :: m_leafn_to_litter_fire(:)       ! (gN/m2/s) from leafn to litter N  due to fire               
   ...
   real(r8), pointer :: leafn_to_npool(:)                   ! leaf N to N pool (gN/m2/s)
   real(r8), pointer :: frootn_to_retransn(:)               ! fine root N to retranslocated N pool (gN/m2/s)
   real(r8), pointer :: frootn_to_npool(:)               ! fine root N to N pool (gN/m2/s)
   ...
   real(r8), pointer :: livestemn_to_npool(:)               ! live stem N to N pool (gN/m2/s)
   ...
   real(r8), pointer :: livecrootn_to_npool(:)              ! live coarse root N to N pool (gN/m2/s)
   ! summary (diagnostic) flux variables, not involved in mass balance
...
!  real(r8), pointer :: xirrig(:)             !irrigation rate
   ...
   real(r8), pointer :: soilpH_sat(:,:)		! pH values calculated based on original pH and acid produced
#endif
...
type, public :: column_cstate_type
  ...
  ! pools for dynamic landcover
   ...
   real(r8), pointer :: ace_prod_col(:)		! (gC/m2) total column carbon in acetate acid 
...
!----------------------------------------------------
! column microbe variables structure
!----------------------------------------------------
#ifdef MICROBE
type, public :: column_microbe_type
   ! column-level methane fluxes
   real(r8), pointer :: cmicbiocs(:,:)			! column-level microbial biomass carbon
   real(r8), pointer :: cdocs_pre(:,:)                      	! column-level dissolved organic carbon
   real(r8), pointer :: cdocs(:,:)                      	! column-level dissolved organic carbon
   ...
   real(r8), pointer :: cmicbions(:,:)			! column-level microbial biomass carbon
   real(r8), pointer :: cdons(:,:)                      	! column-level dissolved organic carbon
   real(r8), pointer :: cdons_unsat(:,:)                   	! column-level dissolved organic carbon in unsatured fraction
   real(r8), pointer :: cdons_sat(:,:)                      	! column-level dissolved organic carbon in saturated fraction
   real(r8), pointer :: cdons_min(:,:)                      	! DON mineralization
   
   real(r8), pointer :: caces(:,:)                      	! column-level acetatic acid
   ...
   real(r8), pointer :: cacebios(:,:)				! column-level biomass of methanogen from acetate
   ...
   real(r8), pointer :: cco2bios(:,:)				! column-level biomass of methanogen from CO2 and H2
   ...
   real(r8), pointer :: caerch4bios(:,:)			! column-level biomass of aerobic methanotrophy 
   ...
   real(r8), pointer :: dochr_vr(:,:)				! column-level decomposition of dissolved organic carbon
   real(r8), pointer :: micbio_hr_vr(:,:,:)			! column-level decomposition of microbial biomass
   real(r8), pointer :: micbio_hr(:,:)			! column-level decomposition of microbial biomass
   ...
   real(r8), pointer :: caces_prod(:,:)                      	! column-level acetatic acid
...
! we set it as part of fine root respiration
   real(r8), Pointer :: cn_microbe(:,:)
    ...
    real(r8), pointer :: ccon_h2s(:,:)			! column-level concentration of H2
...
type, public :: column_nflux_type
   ...
   ! gap mortality
#if(defined MICROBE)
   real(r8), pointer :: litter_n_to_don(:,:)         ! N fluxes associated with to don (gN/m3/s)
#endif
...
!----------------------------------------------------
! gridcell microbe module variables structure
!----------------------------------------------------
#ifdef MICROBE
type, public :: gridcell_microbe_type
   real(r8), pointer :: c_atm(:,:)     ! Atmospheric conc of CH4, O2, CO2 (mol/m3)
...
!#ifdef MICROBE
!   type(column_microbe_type)   :: cmic      !column microbe variables
!#endif
...
!#ifdef MICROBE
!   type(gridcell_microbe_type)   :: gmic    !average of CH4 fluxes all landunit!s
!#endif
