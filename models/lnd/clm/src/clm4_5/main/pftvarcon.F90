...
! for crop
  ...
  !additional parmaeters (DMR 7/26/2013)
  real(r8) mp(0:mxpft)
  real(r8) exudate_frac(0:mxpft)
  !non PFT_specific parameters
...
!-----------------------------------------------------------------------
    ...
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    !New parmaeters added by DMR 7/26/13
...
!    call ncd_io('exudate_frac', exudate_frac, 'read', ncid,readvar=readv)
!    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )  
